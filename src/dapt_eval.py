...
def evalcfg(model_name,split_seed):
    ...
    args=TrainingArguments(output_dir=f"{W}/models/de",per_device_train_batch_size=16,per_device_eval_batch_size=32,
    ...
    return ov
SEEDS=[11,22,33,44]
for name,mn in [("plain CAMeLBERT","CAMeL-Lab/bert-base-arabic-camelbert-mix"),
                ("DAPT CAMeLBERT",f"{W}/models/camelbert-dapt")]:
    s=[evalcfg(mn,sd) for sd in SEEDS]
    print(f"{name:18s} mean={np.mean(s):.3f} std={np.std(s):.3f} runs={[round(x,3) for x in s]}",flush=True)
