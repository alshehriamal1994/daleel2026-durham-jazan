...
with open(f"{W}/preds/task1_test_open_v2.jsonl","w",encoding="utf-8") as f:
    for i,r in enumerate(test):
        f.write(json.dumps({"paragraph_id":r["paragraph_id"],"labels":[L[k] for k in range(6) if P[i,k]],"type":r["type"]},ensure_ascii=False)+"\n")
print("wrote task1_test_open_v2.jsonl counts=",dict(Counter(L[k] for i in range(len(test)) for k in range(6) if P[i,k])),"empty=",int((P.sum(1)==0).sum()))
