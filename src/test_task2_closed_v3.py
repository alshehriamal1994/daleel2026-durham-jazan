...
CAM=f"{W}/models/camelbert-dapt-v2"; MAR=f"{W}/models/marbert-dapt-v2"  # v3: DAPT on all task text
...
def train_family(model_name,seeds):
    tok=AutoTokenizer.from_pretrained(model_name); tf=feats(rows,tok,True); df=feats(testr,tok,False)
    tp=np.zeros(6); tot=0
...
def decode(probs,offs,ths,text):
    ...
    return spans
cam_probs,cam_df=train_family(CAM,[42,1,2,3,4,5,6,7])
mar_probs,mar_df=train_family(MAR,[42,1,2,3,4,5,6,7])
...
with open(f"{W}/preds/task2_test_closed_v3.jsonl","w",encoding="utf-8") as f:
    for o in out: f.write(json.dumps(o,ensure_ascii=False)+"\n")
import collections; c=collections.Counter(s["label"] for o in out for s in o["labels"])
print("wrote task2_test_closed_v3.jsonl n=",len(out)," spans=",dict(c)," avg/para=",round(sum(len(o['labels']) for o in out)/len(out),2))
