...
W=os.environ.get("DALEEL_ROOT", os.path.dirname(os.path.dirname(os.path.abspath(__file__)))); MAXLEN=384; EPOCHS=8; LR=2e-5; BS=16; SEEDS=[42,1,2,3,4,5,6,7]
DAPT=f"{W}/models/camelbert-dapt-v2"  # v4
...
rows=[json.loads(l) for l in open(f"{W}/data/train_task_1.jsonl",encoding="utf-8")]+[json.loads(l) for l in open(f"{W}/data/dev_task_1_ref.jsonl",encoding="utf-8")]  # v2: +217 dev gold
bt1=[json.loads(l) for l in open(f"{W}/data/train_task_1_bt.jsonl",encoding="utf-8")]+[json.loads(l) for l in open(f"{W}/data/dev_task_1_bt.jsonl",encoding="utf-8")]
bt2=[json.loads(l) for l in open(f"{W}/data/train_task_1_bt2.jsonl",encoding="utf-8")]+[json.loads(l) for l in open(f"{W}/data/dev_task_1_bt2.jsonl",encoding="utf-8")]
bt3=[json.loads(l) for l in open(f"{W}/data/train_task_1_bt3.jsonl",encoding="utf-8")]+[json.loads(l) for l in open(f"{W}/data/dev_task_1_bt3.jsonl",encoding="utf-8")]
SYNTH=[json.loads(l) for l in open(f"{W}/data/synth_all.jsonl",encoding="utf-8")]+[json.loads(l) for l in open(f"{W}/data/synth_v2/t1_batch_agent.jsonl",encoding="utf-8")]  # v4: +120 new synth
...
with open(f"{W}/preds/task1_test_open_v4.jsonl","w",encoding="utf-8") as f:
    for i,r in enumerate(test):
        f.write(json.dumps({"paragraph_id":r["paragraph_id"],"labels":[L[k] for k in range(6) if P[i,k]],"type":r["type"]},ensure_ascii=False)+"\n")
print("wrote task1_test_open_v4.jsonl counts=",dict(Counter(L[k] for i in range(len(test)) for k in range(6) if P[i,k])),"empty=",int((P.sum(1)==0).sum()))
