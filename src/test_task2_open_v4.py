...
def train_family(model_name,seeds):
    tok=AutoTokenizer.from_pretrained(model_name); tf=feats(rows+SYNTH,tok,True); df=feats(testr,tok,False)
    tp=np.zeros(6); tot=0
...
with open(f"{W}/preds/task2_test_open_v4.jsonl","w",encoding="utf-8") as f:
    for o in out: f.write(json.dumps(o,ensure_ascii=False)+"\n")
import collections; c=collections.Counter(s["label"] for o in out for s in o["labels"])
print("wrote task2_test_open_v4.jsonl n=",len(out)," spans=",dict(c)," avg/para=",round(sum(len(o['labels']) for o in out)/len(out),2))
