...
BTS=[[json.loads(l) for l in open(f"{W}/data/train_task_1_{t}.jsonl",encoding="utf-8")] for t in ["bt","bt2","bt3"]]
def yv(r):
    y=[0.]*6
...
def build_train(tr,mode):
    tt=[texts[i] for i in tr]; tl=[Y[i].tolist() for i in tr]
    # 1x BT for all
    tt+=[BTS[0][i]["text"] for i in tr]; tl+=[Y[i].tolist() for i in tr]
    if mode=="rare":  # extra 2 BTs for rare-class examples only (oversample)
        for j in [1,2]:
            for i in tr:
                if RARE[i]:
                    tt.append(BTS[j][i]["text"]); tl.append(Y[i].tolist())
    return tt,tl
def evalcfg(ss,mode):
    ...
    a=TrainingArguments(output_dir=f"{W}/models/ra",per_device_train_batch_size=16,per_device_eval_batch_size=32,
...
for name,mode in [("1xBT (baseline)","base"),("+rare-class heavy aug","rare")]:
    s=[evalcfg(sd,mode) for sd in [11,22,33,44]]
    print(f"{name:24s} mean={np.mean(s):.3f} runs={[round(x,3) for x in s]}",flush=True)
