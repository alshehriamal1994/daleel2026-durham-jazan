...
with open(f"{W}/oof/t2_test_probs_open_v5.pkl","wb") as f:
...
with open(f"{W}/preds/task2_test_open_v5.jsonl","w",encoding="utf-8") as f:
    for o in out: f.write(json.dumps(o,ensure_ascii=False)+"\n")
import collections; c=collections.Counter(s["label"] for o in out for s in o["labels"])
print("wrote task2_test_open_v5.jsonl spans=",dict(c)," avg/para=",round(sum(len(o['labels']) for o in out)/len(out),2),flush=True)
