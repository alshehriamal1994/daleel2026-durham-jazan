...
DAPT=f"{W}/models/camelbert-dapt-v2"  # v3
# robust median thresholds from the dev-#1 run (logs_rare3x.txt) — computed from train-only CV, unchanged for test
THS=np.array([0.425,0.25,0.2,0.3,0.125,0.275])
...
with open(f"{W}/preds/task1_test_closed_v3.jsonl","w",encoding="utf-8") as f:
    for i,r in enumerate(test):
        f.write(json.dumps({"paragraph_id":r["paragraph_id"],"labels":[L[k] for k in range(6) if P[i,k]],"type":r["type"]},ensure_ascii=False)+"\n")
print("wrote task1_test_closed_v3.jsonl counts=",dict(Counter(L[k] for i in range(len(test)) for k in range(6) if P[i,k])),"empty=",int((P.sum(1)==0).sum()))
