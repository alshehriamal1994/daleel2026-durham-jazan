...
W="/home/amal/Desktop/daleel2026"; MAXLEN=384; EPOCHS=8; LR=2e-5; BS=16; FOLDS=5; SEEDS=[42,1,2,3,4]
...
extra=json.load(open(f"{W}/data/rare_bt_extra.json"))
...
with open(f"{W}/preds/task1_dev_open.jsonl","w",encoding="utf-8") as f:
...
print("wrote task1_dev_open.jsonl counts=",dict(Counter(L[k] for i in range(len(dev)) for k in range(6) if P[i,k])),"empty=",int((P.sum(1)==0).sum()))
