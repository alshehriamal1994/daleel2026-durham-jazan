...
SYNTH=[json.loads(l) for l in open(f"{W}/data/synth2_all.jsonl",encoding="utf-8")]  # OPEN: LLM synthetic spans
...
with open(f"{W}/preds/task2_test_open_v2.jsonl","w",encoding="utf-8") as f:
    for o in out: f.write(json.dumps(o,ensure_ascii=False)+"\n")
import collections; c=collections.Counter(s["label"] for o in out for s in o["labels"])
print("wrote task2_test_open_v2.jsonl n=",len(out)," spans=",dict(c)," avg/para=",round(sum(len(o['labels']) for o in out)/len(out),2))
