...
Used verbatim in `src/t1_llm_sft.py` (Arabic original there; English translation in the paper appendix).
...
- **Task 2 output format** (*segment-concatenation generation*): the paragraph is authored as an
  **ordered list of labeled segments** `{"type": ..., "segments": [{"t": <text>, "l": <label|O>}]}`;
  unlabeled connective glue (e.g., «و») is allowed between spans with label `O`. The builder
